...
- **Маскирования на уровне вотчеров больше нет.** `input.headers_masking`,
  ...
  перестанет. **Перенесите свои ключи в `masking.keys`**: дефолты покрывают только то,
  что покрывали дефолты раньше (`authorization`, `cookie`, `x-xsrf-token`, `set-cookie`,
  `*token*`, `*password*`), но не `ssn`, `iban`, `card` и прочее, что вы добавляли сами.
- **Вместо них один глобальный список** в `masking.keys`. Опубликованный конфиг теперь
  ...
  секцию к себе, только если хотите её изменить. Дефолты продублированы в коде, в
  `MaskingConfig::DEFAULT_KEYS`, поэтому устаревший кеш конфига не оставит вас совсем
  без маскирования.
...
`API_KEY`, и `lastName`. Совпадение на родительском ключе маскирует всё поддерево:
`auth` маскирует и `auth.method`.

Значение, которое является **строкой с JSON-документом**, разбирается, маскируется и
запаковывается обратно: приложения передают целые документы строками — каст `array` у
Eloquent кладёт такой прямо в изменения модели, — а ключ, под которым лежит эта строка,
ничего не говорит о её содержимом. Разбираются только строки, начинающиеся с `{` или
`[`, а документ, в котором ничего не совпало, остаётся байт в байт, без переупаковки.
