...
- **Вместо них два глобальных списка** — `masking.full_keys` и `masking.partial_keys`.
...
- **Структура данных трейса изменилась** там, где старая форма прятала данные приложения
  от маскера: значения кеша лежат под своим кеш-ключом (`cache.<key>.value`), почтовые
  адреса — под `message` парами `email`/`full_name` вместо «адрес в роли ключа», а
  query-строка отделена от url в `query` и `query_string`. Всё, что читает эти поля на
  приёмной стороне, нужно обновить.
...
Оба списка — подстроки ключа без учёта регистра; два пустых списка выключают
маскирование. Ключ подходит, если *содержит* одну из подстрок, — подходят и
