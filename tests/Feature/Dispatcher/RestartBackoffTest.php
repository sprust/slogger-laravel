...
/**
 * A worker that dies on boot was restarted once a second forever - some 86k restarts
 * and as many state-file writes a day.
 */
class RestartBackoffTest extends BaseTestCase
{
    ...
    public function testAWorkerThatStaysUpLongEnoughClearsItsCount(): void
    {
        ...
        // one that died two seconds in is still running when the loop next looks, and
        // used to clear the count on that alone
    ...
    public function testABackingOffSlotIsSkippedRatherThanSleptOn(): void
    {
        ...
        // put off without stopping the loop, which is what sleep() did - leaving the
        // other slots unwatched and their output unread
        self::assertFalse($mayRefill->invoke($dispatcher, 0));

        self::assertLessThan(1, time() - $startedAt);

        // and stays put off until its deadline, not counting a failure per tick
        self::assertFalse($mayRefill->invoke($dispatcher, 0));
        self::assertFalse($mayRefill->invoke($dispatcher, 0));
