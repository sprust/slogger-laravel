...
class BodySizeCapTest extends BaseWatcherTestCase
{
    ...
    /**
     * A chunked request declares no length, so what was actually collected is measured
     * too - the json path had neither check.
     */
    ...
    /**
     * A multipart length is mostly file bytes, and of a file only the name and size is
     * recorded: measuring it would drop the fields of every upload.
     */
    ...
    /**
     * The xml path measured the request against the *response* cap, so a body well
     * inside `input.max_content_length` was dropped.
     */
